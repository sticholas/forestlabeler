...
# If True, polygon will NOT be added when it contains multiple different species points.
...
class CanopyCrownProfileTool(QgsMapTool):
    ...
    def get_species_from_points_inside_polygon(self, geom_target):
        ...
        for pt_feat in species_layer.getFeatures():
            ...
            # contains is strict; intersects also catches points exactly on the polygon boundary.
            if test_geom.contains(pt_geom) or test_geom.intersects(pt_geom):
        ...
        if len(unique_species) == 1:
            # Allows multiple captured points only when they all have the same species code.
            return unique_species[0], len(matches), None

        return (
            None,
            len(matches),
            "Multiple different species points found inside polygon: " + ", ".join(unique_species)
        )
